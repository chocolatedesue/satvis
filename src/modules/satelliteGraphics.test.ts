...
const GEO_PERIOD_MIN = 1436;

describe("isLeo", () => {
  test("LEO period is within two hours", () => {
    expect(isLeo(ISS_PERIOD_MIN)).toBe(true);
    expect(isLeo(120)).toBe(true);
    expect(isLeo(121)).toBe(false);
    expect(isLeo(GEO_PERIOD_MIN)).toBe(false);
  });
});
...
describe("groundTrackDescription", () => {
  test("converts the swath width to meters", () => {
    expect(groundTrackDescription(ISS_PERIOD_MIN, 290)).toEqual({ widthMeters: 290000 });
  });

  test("is unavailable for non-LEO satellites", () => {
    expect(groundTrackDescription(GEO_PERIOD_MIN, 290)).toBeUndefined();
  });
});

describe("coneDescription", () => {
  test("converts the FOV to a half angle in radians", () => {
    const description = coneDescription(ISS_PERIOD_MIN, 45);
  ...
  test("is unavailable for non-LEO satellites", () => {
    expect(coneDescription(GEO_PERIOD_MIN, 45)).toBeUndefined();
  });
});
