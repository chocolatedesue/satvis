...
#
# No depth or quality option: both decide the bytes of levels 0-2, which are
# committed, so both are constants in tiles.py (MAX_ZOOM, QUALITY) rather than flags
# that could rewrite tracked files as a side effect of an experiment.
#
