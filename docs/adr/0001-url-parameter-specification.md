...
| `walker`     | `sat.walker`               | string list         | comma-joined Walker patterns, each `i:T/P/F@altKm` with optional `~raanSpan` and `+raanOffset`; empty means none generated                                    | empty           |
| `cluster`    | `sat.cluster`              | string list         | comma-joined free-flying formations, each `i:ringsXpitchM@altKm` with optional `+raanOffset`; empty means none generated                                      | empty           |
| `mig`        | `sat.migration`            | boolean             | `true` \| `false` — the naive KV-cache live-migration overlay                                                                                                 | `false`         |
| `migst`      | `sat.migrationStages`      | number choice       | `1` \| `2` \| `4` \| `6` \| `8` — how many stages the migrating pipeline is cut into                                                                          | `4`             |
| `migpol`     | `sat.migrationPolicy`      | enum                | `predictive` \| `naive` — the live-migration policy (`predictive` pre-empts eclipse entry)                                                                    | `naive`         |
| `miginc`     | `sat.migrationIncremental` | boolean             | `true` \| `false` — incremental KV sync, shipping only the cache's growth after the first snapshot                                                            | `false`         |
| `links`      | `sat.links`                | boolean             | `true` \| `false` — the stable-constellation link overlay over generated Walker patterns                                                                      | `true`          |
| `mark`       | `sat.marks`                | string list         | comma-joined cluster tokens, each `<plane>-<slot>@<wire>` with 1-based plane/slot (`1-1@53:40/4/1@550`); members get a halo and slot label, every pair bonded | empty           |
