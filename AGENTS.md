...
- Entrypoints: `index.html`, `ot.html`, `embedded.html`, `test.html` (all configured as Vite MPA inputs), plus `public/404.html`. `ot.html` is a near-copy of `index.html` and exists so `/ot` is a real file — see the not-found note below.
