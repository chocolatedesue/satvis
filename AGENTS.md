...
- **Git submodules**: `data/models` (3D models) is now the only one; run `git submodule update --init` before first build. **`git worktree add` does not populate submodules**, so a fresh worktree has it empty, and the 3D models have no fallback for that yet.
  - **`data/cesium-assets` was removed.** It served the base map and the star maps; both are generated now. The two things that still wanted it were self-checks rather than inputs — `scripts/imagery/generate.sh` compares its tiles against the old tileset, `scripts/starmap/generate.sh` correlates its faces against the Tycho ones — and both read the reference only _after_ writing their output, so neither can change a byte of what is generated. Keeping a submodule everyone had to initialise for two checks most people never run was the wrong trade; they now look for an optional clone at **`scripts/.reference/cesium-assets`** (gitignored, clone command in each script's note and in `scripts/.reference/.gitignore`) and skip with a message when it is absent.
