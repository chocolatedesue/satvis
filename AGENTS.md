...
- **`docs/orbital-compute.md`** — what an orbit gives a compute cluster: the
  geometry, energy and inference layers, and why all three point at the same
  altitude and inclination. Start here for "which orbit should this go in".
- **`docs/compute-capacity.md`** — the capacity study in paper form: the serving
  conjunction, host selection, four baselines (`pnpm orbit-lab evaluate`) and seven
  ablations, with the threats to validity. Start here for "how many shells, how deep".
- **`docs/TODO.md`** — what is done and what is next, for the orbital-compute
  work specifically. The open items in section 3 are the research queue.
- **`docs/HANDOFF.md`** — the same material written for a reader arriving cold:
  repo, live URLs, architecture by layer, verification commands, key files.
- **`docs/usage-zh.md`** — the Chinese end-user guide: the interface, the nine
  `?demo=` scenes, common URL combinations and the recurring user questions.
- **`docs/starlink-energy-report.md`** — generated by `pnpm energy-report`. Do not
  edit; recompute.
...
- `src/modules/util/` is Cesium-free and Vue-free on purpose, which is what lets
  `scripts/research/` run under node with no build step — `pnpm orbit-lab orbit 550 53`
  answers the same questions the orbit lab panel does, from a terminal.
- **`scripts/` has three kinds of script, kept apart because they are run at
  different times by different things**: `scripts/research/` is the closed-form
  and SGP4 analysis (`orbit-lab.ts` and the studies it can call,
  `derive-isl-topology.ts`), `scripts/verify/` drives a _built_ site with
  headless browsers, and `scripts/assets/` holds the containerised asset
  generators (`generate.sh` plus one context dir per generator).
  `scripts/deploy-pages.sh` is the one deployment entry point. Research does not
  need the app: a study is a script plus a `.test.ts` beside the module it
  exercises.
...
- `pnpm orbit-lab <command>` runs closed-form orbit analysis in the terminal, with
  no browser — `orbit`, `shells`, `clusters`, `formation`, `design`, `capacity`,
  `evaluate`. See README, "Orbit analysis without a globe".
...
- **A module a script can reach must import with its `.ts` extension.**
  `scripts/research/` load `src/modules/util/*.ts` through node's own type
  stripping, which
...
- **Everything under `data/` ships.** The static-copy glob takes it wholesale,
  which is why the generators live under `scripts/assets/` rather than in `data/`.

## Deployment

The fork's public site is GitHub Pages: `.github/workflows/deploy-pages.yml`
publishes the worker-less shape on every push to `main`. The worker-backed target
below is optional — `pnpm deploy` builds the frontend and deploys the worker,
which needs a KV namespace bound as `GP_KV` (`worker/wrangler.jsonc`). After a first deploy KV is
