...
| Build the base map | `pnpm update-imagery` (docker; writes the gitignored `data/imagery/` tiles) |
...
- **Data assets**: `data/` also contains the generated Cesium assets (base map, star maps) and 3D-model plugins under `data/custom/`. Copied into `dist/` at build time via `vite-plugin-static-copy` — except `data/cesium-assets`, which is excluded there because nothing loads from it any more.
  - **`data/imagery/`** holds the `OfflineHighres` base map: a geodetic TMS pyramid of Natural Earth II, levels 0–5 as 256px WebP, 17.5 MB, built by `pnpm update-imagery`. Same arrangement as `data/starmap/` below — gitignored contents, tracked directory, generator under `scripts/`. The generator applies the colour grade the retired `data/cesium-assets` tileset was cut with (`RECOLOR` in `scripts/imagery/tiles.py`), because the raw public source is 48 levels per channel brighter and would clash with the bundled `Offline` layer it falls back to; a run finishes by checking itself against that old tileset when the submodule is present, and agrees to ~3 levels, which is the lossy-encoding floor. Nothing in the app names the tile format — Cesium reads the extension out of `tilemapresource.xml`, so the generator is the only place that decides it.
    - **Levels 0–3 are precached, 4 and 5 are not.** That is 170 tiles and 1.4 MB against 17.5 MB for the whole pyramid, and it is the line between "the globe is correct offline wherever you turn it" and "zoomed-in detail, which the CacheFirst rule already keeps for wherever you have been". Levels 0–2 are precached even though Cesium's own `Offline` covers the same ground at the same resolution: that is a different layer at a different url, and Cesium walks _this_ pyramid from its root, so a precache starting at 3 would leave holes at the top.
    - **A build with no tiles is a normal build, not a failure.** They are generated, so a checkout that has not run the generator simply omits them, the precache glob matches nothing, and the app falls back at runtime. `vite.config.ts` warns (`satvis-imagery-notice`) rather than failing, because CI legitimately has no tiles — but `pnpm deploy` runs `pnpm build` directly, so that warning is the only thing standing between a forgotten `pnpm update-imagery` and a deploy without the default base map.
...
- **Git submodules**: `data/models` (3D models) is the only one the app still loads from; run `git submodule update --init` before first build. **`git worktree add` does not populate submodules**, so a fresh worktree has both of them empty, and the 3D models have no fallback for that yet.
  - **`data/cesium-assets` is on its way out and nothing at runtime reads it.** The base map it used to serve is now built by `pnpm update-imagery`, the star maps by `pnpm update-starmap`, and `vite.config.ts` excludes the submodule from the copy into `dist/`. What keeps it declared is that both generators check themselves against it — `scripts/imagery/generate.sh` mounts the old `NaturalEarthII` tileset as the reference for its colour comparison, `scripts/starmap/generate.sh` mounts the Tycho faces for its orientation check — and each skips its check with a note when the submodule is absent. Retiring it entirely means giving up those two references, so it stays until someone decides that trade is worth making.
  - **The `OfflineHighres` probe.** The layer is now missing whenever nobody has run `pnpm update-imagery`, which is the common case in a fresh checkout, so it probes `data/imagery/NaturalEarthII/tilemapresource.xml` and switches the selection to the bundled `Offline` layer with a console warning when it is not there. The probe exists because Cesium cannot report the failure — `TileMapServiceImageryProvider.fromUrl` treats a missing `tilemapresource.xml` as "carry on with defaults" and resolves happily, then 404s every tile behind a blank globe. It **reads** the manifest rather than checking the status, because nothing serving this app answers a missing file with an error status: the dev server falls back to `index.html` with a 200, and the deployed Worker used to as well. Checking `response.ok` alone made the probe pass on exactly the checkouts it was written for, and it still would under `pnpm dev`.
